...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function _log_request($authorized = FALSE)
    {
        ...
        $is_inserted = $this->rest->db
            ->insert(
                ...
                'vURI' => $this->uri->uri_string(),
                'vMethod' => $this->request->method,
                'tParams' => $this->_args ? ($this->config->item('rest_logs_json_params') === TRUE ? json_encode($this->_args) : serialize($this->_args)) : NULL,
                'vApiKey' => isset($this->rest->key) ? $this->rest->key : '',
                'vIpAddress' => $this->input->ip_address(),
                'iTime' => time(),
    ...
    {
        $payload['fRTime'] = $this->_end_rtime - $this->_start_rtime;

        return $this->rest->db->update(
                $this->config->item('rest_logs_table'), $payload, [
                'iLogId' => $this->_insert_id
            ]);
    }
    ...
    {
        $payload['iResponseCode'] = $http_code;
